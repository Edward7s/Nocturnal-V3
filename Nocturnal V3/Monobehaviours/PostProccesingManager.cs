...
namespace Nocturnal.Monobehaviours
{
    internal class PostProccesingManager :MonoBehaviour
    {
        ...
        public void UpdateExposure(float val) => _colorGradinat.postExposure.Override(-val * 10);
        public void UpdateSaturation(float val) => _colorGradinat.saturation.Override(val * 60);
        public void UpdateTint(float val) => _colorGradinat.tint.Override(val * 60);
        public void UpdateTemperature(float val) => _colorGradinat.temperature.Override(val * 60);
        public void gammaX(float val) => _colorGradinat.gamma.Override(new Vector4(val * 4, Settings.ConfigVars.Gamma[1] * 4, Settings.ConfigVars.Gamma[2] * 4, 0));
        public void gammaY(float val) => _colorGradinat.gamma.Override(new Vector4(Settings.ConfigVars.Gamma[0] * 4, val * 4, Settings.ConfigVars.Gamma[2] * 4, 0));
        public void gammaZ(float val) => _colorGradinat.gamma.Override(new Vector4(Settings.ConfigVars.Gamma[0] * 4, Settings.ConfigVars.Gamma[1] * 4, val * 4, 0));
        public void UpdateBloom(float val) => _bloom.intensity.Override(val * 10);
